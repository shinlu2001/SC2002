...
public class IdGenerator {
    ...
    /**
     * Initializes the next available IDs based on the maximum ID found in the
     * loaded data.
     * This should be called once after loading all data from CSV files.
     */
    public static void initializeIds(List<User> users, List<Project> projects, List<BTOApplication> applications,
            List<Enquiry> enquiries, List<Registration> registrations) {
    ...
    public static int nextReportId() {
        return nextReportId++;
    } // Added getter for Report IDs

    public static int nextReceiptId() {
     ...
     */
    public static void ensureApplicationIdAtLeast(int id) {
        if (id >= nextApplicationId) {
            nextApplicationId = id + 1;
        }
    }

    /**
     * Ensures the next registration ID is at least the given value
     * 
     * @param id The minimum value for the next registration ID
     */
    public static void ensureRegistrationIdAtLeast(int id) {
        if (id >= nextRegistrationId) {
            nextRegistrationId = id + 1;
        }
    }

    /**
     * Ensures the next enquiry ID is at least the given value
     * 
     * @param id The minimum value for the next enquiry ID
     */
    public static void ensureEnquiryIdAtLeast(int id) {
        if (id >= nextEnquiryId) {
            nextEnquiryId = id + 1;
        }
    }
    ...
    /**
     * Ensures the next flat ID is at least the given value
     * 
     * @param id The minimum value for the next flat ID
     */
    public static void ensureFlatIdAtLeast(int id) {
        if (id >= nextFlatId) {
            nextFlatId = id + 1;
        }
    }
